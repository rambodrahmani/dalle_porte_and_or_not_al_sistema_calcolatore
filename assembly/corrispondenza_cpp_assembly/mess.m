 ...
 *(.rela.dtors)
 *(.rela.got)
 .rela.got      0x00000000004000e8        0x0 mess.o
 ...
 *(.rela.iplt)
 .rela.iplt     0x00000000004000e8        0x0 mess.o
 ...
 *(.plt)
 *(.iplt)
 .iplt          0x0000000000401000        0x0 mess.o
 ...
 *(.got)
 .got           0x0000000000402000        0x0 mess.o
 *(.igot)
                0x0000000000402000                . = DATA_SEGMENT_RELRO_END (., (SIZEOF (.got.plt) >= 0x18)?0x18:0x0)

.got.plt        0x0000000000402000        0x0
 *(.got.plt)
 .got.plt       0x0000000000402000        0x0 mess.o
 *(.igot.plt)
 .igot.plt      0x0000000000402000        0x0 mess.o

.data           0x0000000000402000       0x16
 *(.data .data.* .gnu.linkonce.d.*)
 .data          0x0000000000402000       0x16 mess.o

.data1
 *(.data1)
                0x0000000000402016                _edata = .
                [!provide]                        PROVIDE (edata = .)
                0x0000000000402016                . = .
                0x0000000000402016                __bss_start = .

.bss            0x0000000000402016        0x0
 *(.dynbss)
 *(.bss .bss.* .gnu.linkonce.b.*)
 .bss           0x0000000000402016        0x0 mess.o
 *(COMMON)
                0x0000000000402016                . = ALIGN ((. != 0x0)?0x8:0x1)

.lbss
 *(.dynlbss)
 *(.lbss .lbss.* .gnu.linkonce.lb.*)
 *(LARGE_COMMON)
                0x0000000000402018                . = ALIGN (0x8)
                0x0000000000402018                . = SEGMENT_START ("ldata-segment", .)

.lrodata
 *(.lrodata .lrodata.* .gnu.linkonce.lr.*)

.ldata          0x0000000000404018        0x0
 *(.ldata .ldata.* .gnu.linkonce.l.*)
                0x0000000000404018                . = ALIGN ((. != 0x0)?0x8:0x1)
                0x0000000000404018                . = ALIGN (0x8)
                0x0000000000402018                _end = .
                [!provide]                        PROVIDE (end = .)
                0x0000000000404018                . = DATA_SEGMENT_END (.)
